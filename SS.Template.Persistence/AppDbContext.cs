...
namespace SS.Template.Persistence
{
    public class AppDbContext : DbContext
    {
        ...
        {
            return this.Products.Where(p => p.Category == category).ToList();
        }
        public IEnumerable<Product> GetProductById(int id)
        {
            return this.Products.Where(p => p.Id == id).ToList();
        }
        ...
        {
            Product prod = this.Products.First(p => p.Id == id);
            this.Remove(prod);
            SaveChanges();
        }
        public void DeleteOrder(int id)
        {
            Order orders = this.Orders.First(p => p.Id == id);
            this.Remove(orders);
            SaveChanges();
        }
    }
